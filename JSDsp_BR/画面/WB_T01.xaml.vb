Public Class WB_T01
    ...
    Private Sub Line色変更()
        ...
        For c = 1 To 8
            CType(FindName("LN_1R_" & c & "V"), Line).Stroke = Brushes.White
            CType(FindName("LN_1R_" & c & "H"), Line).Stroke = Brushes.White
        Next c
    ...
    Public Sub 初期化()
        ...
        For s = 1 To 16
            ...
            CType(FindName("TB_1R_" & ss), TextBox).Opacity = 0
            CType(FindName("TB_1R_" & ss), TextBox).Text = ""
            CType(FindName("TB_1R_" & ss), TextBox).Background = Brushes.White
            CType(FindName("TB_1R_" & ss), TextBox).Foreground = Brushes.Black
        Next s
        ...
        'LINE
        For c = 1 To 8
            CType(FindName("LN_1R_" & c & "V"), Line).Opacity = 0
            CType(FindName("LN_1R_" & c & "H"), Line).Opacity = 0
        Next c
    ...
    Public Sub データセット(SC_BG_Startlist As SC_BG_Startlist_C, SC_BR_Result As SC_BR_Result_C, カテゴリ番号 As String)
        ...
        For i = 1 To UBound(SC_BG_Startlist.SC_BG_Startlist_選手) - 1
            ...
            If SC_BG_Startlist.SC_BG_Startlist_選手(i).背番号 IsNot Nothing Then
                ...
                CType(FindName("TB_1R_" & TB番号), TextBox).Text = SC_BG_Startlist.SC_BG_Startlist_選手(i).リーダー名
            End If
        Next i
            ...
            For i = 1 To UBound(SC_BR_Result.SC_BR_Result_結果)
                ...
                If Strings.Right(SC_BR_Result.SC_BR_Result_結果(i).区分番号, 1) = CStr(k) And
                   ...
                   SC_BR_Result.SC_BR_Result_結果(i).カテゴリ番号 = カテゴリ番号 Then
                    ...
                    Dim 勝TB番号 As String = CInt(SC_BR_Result.SC_BR_Result_結果(i).勝者背番号).ToString("00")

                    '勝った方
                    CType(FindName("TB_1R_" & 勝TB番号), TextBox).Foreground = Brushes.Black
                    CType(FindName("TB_1R_" & 勝TB番号), TextBox).Background = Brushes.White

                    '2ラウンドのTextboxにも記入
                    Dim 次区分番号 As String = ((勝TB番号 + 1) \ 2).ToString("00")

                    CType(FindName("TB_2R_" & 次区分番号), TextBox).Text = SC_BR_Result.SC_BR_Result_結果(i).勝者リーダー名



                    '負けた方
                    Dim 負背番号 As String = "00"
                    If CInt(勝TB番号) Mod 2 = 0 Then
                        負背番号 = (CInt(SC_BR_Result.SC_BR_Result_結果(i).勝者背番号) - 1).ToString("00")
                    Else
                        負背番号 = (CInt(SC_BR_Result.SC_BR_Result_結果(i).勝者背番号) + 1).ToString("00")
                    End If

                    CType(FindName("TB_1R_" & 負背番号), TextBox).Foreground = Brushes.DimGray
                    CType(FindName("TB_1R_" & 負背番号), TextBox).Background = Brushes.Gray
        ...
        For k = 1 To 4
            For i = 1 To UBound(SC_BR_Result.SC_BR_Result_結果)
                ...
                If Strings.Right(SC_BR_Result.SC_BR_Result_結果(i).区分番号, 1) = CStr(k) And
                   SC_BR_Result.SC_BR_Result_結果(i).ラウンド番号 = "020" And
                   SC_BR_Result.SC_BR_Result_結果(i).カテゴリ番号 = カテゴリ番号 Then
    ...
    Public Sub No1()
        ...
        For s = 1 To 16
            Dim ss As String = Format(s, "00")
            フェードイン(CType(FindName("TB_1R_" & ss), TextBox), sb, 0)
        Next s
        ...
        'LINE
        For c = 1 To 8
            フェードイン(CType(FindName("LN_1R_" & c & "V"), Line), sb, 0)
            フェードイン(CType(FindName("LN_1R_" & c & "H"), Line), sb, 0)
        Next c
    ...
    Public Sub 全フェードアウト()
        ...
        For s = 1 To 16
            Dim ss As String = Format(s, "00")
            フェードアウト(CType(FindName("TB_1R_" & ss), TextBox), sb, 0)
        Next s
        ...
        'LINE
        For c = 1 To 8
            フェードアウト(CType(FindName("LN_1R_" & c & "V"), Line), sb, 0)
            フェードアウト(CType(FindName("LN_1R_" & c & "H"), Line), sb, 0)
        Next c
