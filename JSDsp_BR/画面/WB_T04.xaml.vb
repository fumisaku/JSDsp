Public Class WB_T04
    ...
    Private Sub Line色変更()
        ...
        Next c
        For c = 1 To 2
            CType(FindName("LN_SF_" & c & "H"), Line).Stroke = Brushes.White
        Next c
        ...
        CType(FindName("LN_FF_2V"), Line).Stroke = Brushes.White
        CType(FindName("LN_FF_3V"), Line).Stroke = Brushes.White
    ...
    Public Sub 初期化()
        ...
        Next c
        For c = 1 To 2
            CType(FindName("LN_SF_" & c & "H"), Line).Opacity = 0
        Next c


        CType(FindName("LN_FF_1H"), Line).Opacity = 0
        CType(FindName("LN_FF_1V"), Line).Opacity = 0
        CType(FindName("LN_FF_2V"), Line).Opacity = 0
        CType(FindName("LN_FF_3V"), Line).Opacity = 0
    ...
    Public Sub データセット(SC_BG_Startlist As SC_BG_Startlist_C, SC_BR_Result As SC_BR_Result_C, カテゴリ番号 As String)
        ...
        For i = 1 To 4 'UBound(SC_BG_Startlist.SC_BG_Startlist_選手) - 1

            If SC_BG_Startlist.SC_BG_Startlist_選手(i).背番号 IsNot Nothing Then
                Dim TB番号 As String = i.ToString("00")

                CType(FindName("TB_SF_" & TB番号), TextBox).Text = SC_BG_Startlist.SC_BG_Startlist_選手(i).リーダー名
            End If
        Next i
            ...
            For i = 1 To UBound(SC_BR_Result.SC_BR_Result_結果)
                ...
                If Strings.Right(SC_BR_Result.SC_BR_Result_結果(i).区分番号, 1) = CStr(k) And
                   ...
                   SC_BR_Result.SC_BR_Result_結果(i).カテゴリ番号 = カテゴリ番号 Then
                    ...
                    '次ラウンドのTextboxにも記入
                    Dim 次区分番号 As String = ((CInt(勝TB番号) + 1) \ 2).ToString("00")
                    If 次区分番号 <> "00" Then
        ...
        '2人の時は、SFの結果 に出場選手名を入れる
        For i = 1 To 2 'UBound(SC_BG_Startlist.SC_BG_Startlist_選手) - 1

            If SC_BG_Startlist.SC_BG_Startlist_選手(i).背番号 IsNot Nothing Then
                Dim TB番号 As String = i.ToString("00")

                CType(FindName("TB_FF_" & TB番号), TextBox).Text = SC_BG_Startlist.SC_BG_Startlist_選手(i).リーダー名
            End If
        Next i
    ...
    Public Sub No1()
        ...
        Next c
        For c = 1 To 2
            '    フェードイン(CType(FindName("LN_SF_" & c & "H"), Line), sb, 0)
        Next c

        フェードイン(CType(FindName("LN_FF_1H"), Line), sb, 0)
        フェードイン(CType(FindName("LN_FF_1V"), Line), sb, 0)
        フェードイン(CType(FindName("LN_FF_2V"), Line), sb, 0)
        フェードイン(CType(FindName("LN_FF_3V"), Line), sb, 0)
    ...
    Public Sub 全フェードアウト()
        ...
        Next c
        For c = 1 To 2
            フェードアウト(CType(FindName("LN_SF_" & c & "H"), Line), sb, 0)
        Next c

        フェードアウト(CType(FindName("LN_FF_1H"), Line), sb, 0)
        フェードアウト(CType(FindName("LN_FF_1V"), Line), sb, 0)
        フェードアウト(CType(FindName("LN_FF_2V"), Line), sb, 0)
        フェードアウト(CType(FindName("LN_FF_3V"), Line), sb, 0)
